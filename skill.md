...
1. Python 3.8+，安装依赖：`pip install requests`
2. Chrome/Edge 已登录 B站（用于自动提取 Cookie，见下方说明）

## 获取 Cookie

**方式一：自动提取（需 CDP 代理）**

`load_cookies_from_cdp()` 会自动启动浏览器并提取 Cookie。默认依赖同级的 `web-access/scripts/cdp-proxy.mjs`，若不存在请用方式二。
